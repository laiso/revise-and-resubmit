...
"""原稿を一部分ずつ提示し、次へ進む前に読者のログを保存する。

サーバーモード:
  python3 feed.py serve <draft> --run <run-dir> --readers mind-tail,slob-police
  読者には自分のREADER_FEEDアドレスだけを渡す。
  python3 feed.py start
  python3 feed.py next --log "その時点の理解・根拠・推測・未解決"
  python3 feed.py quit --log "最後の理解と停止理由"
  進行役はprogress/close --admin <address>で状態と保存を管理する。

ファイルモード:
  python3 feed.py start <draft> --session <dir> --persona <name> --lang ja
  python3 feed.py next <dir> --log "<entry>"
  python3 feed.py quit <dir> --log "<entry>"
  python3 feed.py transcript <dir>

読者はフィードだけを使い、原稿・状態ファイルを直接読まない。
待機時間は早送り防止の制約であり、人間の読書速度の測定ではない。
"""
from language import add_language, count, unit, ja_chunks, ja_sentences
import argparse
import json
import os
import re
...
MAX_WORDS = 200     # operational chunk bound


def chunk(text, lang="en"):
    if lang == "ja":
        passages = ja_chunks(text)
        # Expose the opening before later sentences can repair its framing.
        opening, remaining = [], 3
        while passages and remaining:
            sentences = ja_sentences(passages.pop(0))
            opening.extend(sentences[:remaining])
            rest = sentences[remaining:]
            remaining -= min(remaining, len(sentences))
            if rest:
                passages.insert(0, "".join(rest))
        return opening + passages
...
BRIEFING = ("This is an incremental understanding probe, not a test of whether "
            "you finish or like the article. Use only your persona's prior knowledge, "
            "text released so far, and your own previous notes. Before advancing, "
            "record in your own words: current understanding (who does what, to "
            "what, in which situation); basis (short textual evidence versus "
            "prior knowledge or inference); how the new material connects to the "
            "title or previous topic and what it adds; open questions or unresolved "
            "referents; "
            "and revisions to earlier interpretations, with the trigger. None is "
            "a valid answer for uncertainties or revisions. Do not invent problems "
            "or supply the author's intended thesis. Preserve initial readings "
            "even when later text repairs them. Fluency and continuation do not "
            "prove comprehension. Interest is a secondary observation. "
            "Do not search, open links, inspect future text, or read source/state "
            "files. You may stop; a requested scope boundary is not abandonment. "
            "Report only at the resolution actually exposed, never invent a "
            "clause-level reaction from a whole sentence.")


def chunk_header(i, chunks, lang="en"):
    remaining = sum(count(c, lang) for c in chunks[i + 1:])
    return f"--- CHUNK {i + 1}/{len(chunks)} ({remaining} {unit(lang)} remain after this) ---"
...
def dwell_floor(text, factor, lang="en"):
    # Operational anti-racing floor; not a model of human reading speed.
    return max(2.0, count(text, lang) * factor) if factor > 0 else 0.0


# ---------------------------------------------------------------- served mode

class Feed:
    def __init__(self, draft, run_dir, names, persona_titles, dwell, lang="en"):
        self.lang = lang
        text = Path(draft).read_text(encoding="utf-8")
        self.chunks = chunk(text, lang)
        self.words = count(text, lang)
        ...
        for name in names:
            if (self.run_dir / name).exists():
                raise FileExistsError(f"Session already exists: {self.run_dir / name}")
        for name in names:
    ...
    def _payload(self, r):
        ...
        return {"header": chunk_header(i, self.chunks, self.lang), "text": self.chunks[i],
    ...
    def start(self, r):
        ...
        p["briefing"] = (f"SESSION {r['name']} | persona: {r['persona']} | "
                         f"{len(self.chunks)} chunks, {self.words} {unit(self.lang)} total\n{BRIEFING}") if first else \
    ...
    def _record(self, r, entry, quit_, check_dwell=True, chunk=None):
        ...
        if not entry:
            return {"refused": "A log entry is required before advancing or stopping. Record your current understanding or reason to stop."}
        ...
        if check_dwell and r["served_at"] is not None and self.dwell > 0:
            floor = dwell_floor(self.chunks[r["cursor"]], self.dwell, self.lang)
            waited = time.time() - r["served_at"]
            if waited < floor:
                return {"refused": f"Operational waiting period: {floor:.0f}s for this passage "
    ...
    def quit(self, r, entry):
        if r["started"] is None:
            return {"error": "Call feed.py start first."}
        if r.get("final_logged") or (r["done"] and r["cursor"] < len(self.chunks)):
            return {"error": "Session already finished."}
        ...
        if r["quit_at"]:
            return {"end": f"Reader stopped at chunk {r['quit_at']}/{len(self.chunks)}. "
                           "The stop point and its reason have been recorded."}
    ...
    def flush(self):
        ...
        for r in self.readers.values():
            state = {"draft": "(served)", "persona": r["persona"], "chunks": self.chunks,
                     ...
                     "started": r["started"] or time.time(), "final_logged": r.get("final_logged", False)}
            if self.lang != "en":
                state["language"] = self.lang
...
def cmd_serve(args):
    ...
    feed = Feed(args.draft, args.run, names, titles, args.dwell, args.lang)
    httpd = ThreadingHTTPServer(("127.0.0.1", args.port), Handler)
    Handler.feed = feed
    Handler.server_ref = httpd
    port = httpd.server_address[1]
    base = f"http://127.0.0.1:{port}"
    print(f"FEED serving {len(feed.chunks)} passages, {feed.words} {unit(feed.lang)}, readers: {', '.join(names)}")
...
def show(state, i):
    chunks = state["chunks"]
    print(chunk_header(i, chunks, state.get("language", "en")))
    print(chunks[i])
    print(chunk_footer())
...
def cmd_start(args):
    ...
    d = Path(args.session)
    if d.exists():
        sys.exit(f"Session already exists: {d}")
    text = Path(args.draft).read_text(encoding="utf-8")
    chunks = chunk(text, args.lang)
    ...
    print(f"SESSION {d} | persona: {state['persona']} | {len(chunks)} chunks, "
          f"{count(text, args.lang)} {unit(args.lang)} total")
    print(BRIEFING)
    show(state, 0)
...
def record(d, state, entry, quit_=False):
    if not entry.strip():
        sys.exit("REFUSED: A log entry is required before advancing or stopping.")
    with (d / "log.jsonl").open("a") as f:
        f.write(json.dumps({"chunk": min(state["cursor"] + 1, len(state.get("chunks", [])) or state["cursor"] + 1), "quit": quit_,
...
def cmd_quit(args):
    ...
    _session_or_die(args)
    d, state = load(args.session)
    if state.get("final_logged") or (state["done"] and state["cursor"] < len(state["chunks"])):
        sys.exit("Session already finished.")
...
def main():
    ...
    s.add_argument("--dwell", type=float, default=0.08,
                   help="seconds per word (en) or character (ja); an operational floor, not measured reading speed; a reader must spend on a passage before advancing; 0 disables")
